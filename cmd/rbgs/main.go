...
func main() {
	var (
		...
		enablePprof bool
		pprofAddr   string
		// v1alpha1 compatibility (indirect workloads: Deployment, StatefulSet, LeaderWorkerSet)
	...
	flag.BoolVar(
		&enableV1Alpha1Compat, "enable-v1alpha1-compat", true,
		"Enable v1alpha1 compatibility mode for indirect workload types "+
	...
	mgr, err := ctrl.NewManager(
		restConfig, newManagerOptions(webhookMode, webhookServer, metricsServerOptions, probeAddr, enableLeaderElection, enableV1Alpha1Compat),
	)
	if err != nil {
	...
	rbgReconciler, err := workloadscontroller.NewRoleBasedGroupReconciler(mgr, scheduler.SchedulerPluginType(schedulerName), nodeBindings, enableV1Alpha1Compat)
	if err != nil {
...
func newManagerOptions(webhookMode string, webhookServer webhook.Server, metricsOpts metricsserver.Options, probeAddr string, enableLeaderElection bool, enableV1Alpha1Compat bool) ctrl.Options {
	opts := ctrl.Options{
		Scheme:                 scheme,
		Metrics:                metricsOpts,
		...
		Cache:                  cacheOptions(enableV1Alpha1Compat),
	}
	if !webhooksEnabled(webhookMode) {
...
func cacheOptions(enableV1Alpha1Compat bool) cache.Options {
	...
	// Only cache StatefulSet and Deployment when v1alpha1 compat is enabled.
	if enableV1Alpha1Compat {
