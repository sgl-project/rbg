...
// HasPortAllocatorConfigFromAnnotations checks if the given annotations map has port allocator config
func HasPortAllocatorConfigFromAnnotations(annotations map[string]string) bool {
	if annotations == nil {
		return false
	}
	_, exists := annotations[PortAllocatorAnnotationKey]
	return exists
}

// ParsePortAllocatorConfigFromAnnotations parses the PortAllocatorConfig from annotations map
func ParsePortAllocatorConfigFromAnnotations(annotations map[string]string) (*PortAllocatorConfig, error) {
	if annotations == nil {
		return nil, nil
	}

	annotationValue, exists := annotations[PortAllocatorAnnotationKey]
	if !exists || annotationValue == "" {
		return nil, nil
	}
