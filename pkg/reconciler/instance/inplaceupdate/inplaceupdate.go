...
import (
	...
	clientdapter "sigs.k8s.io/rbgs/pkg/inplace/pod/clientdapter"
...
type Interface interface {
	CanUpdateInPlace(oldRevision, newRevision *apps.ControllerRevision, opts *podinplaceupdate.UpdateOptions) bool
	Refresh(pod *v1.Pod, opts *podinplaceupdate.UpdateOptions) podinplaceupdate.RefreshResult
	Update(pod *v1.Pod, oldRevision, newRevision *apps.ControllerRevision, opts *podinplaceupdate.UpdateOptions) podinplaceupdate.UpdateResult
}
...
func (c *realControl) CanUpdateInPlace(oldRevision, newRevision *apps.ControllerRevision, opts *podinplaceupdate.UpdateOptions) bool {
	componentRevisions, err := c.groupComponentControllerRevision(oldRevision, newRevision)
	if err != nil {
		return false
	}
...
func (c *realControl) Refresh(pod *v1.Pod, opts *podinplaceupdate.UpdateOptions) podinplaceupdate.RefreshResult {
	return c.inplaceControl.Refresh(pod, opts)
}

func (c *realControl) Update(pod *v1.Pod, oldRevision, newRevision *apps.ControllerRevision, opts *podinplaceupdate.UpdateOptions) podinplaceupdate.UpdateResult {
	componentsRvHistory, err := c.groupComponentControllerRevision(oldRevision, newRevision)
	if err != nil {
...
func (c *realControl) groupComponentControllerRevision(oldRevision, newRevision *apps.ControllerRevision) (map[string]revisionHistory, error) {
	oldRolesRevisions, err := c.splitComponentControllerRevision(oldRevision)
	if err != nil {
		klog.Errorf("fail to split old role controllerRevision: %v, err: %v", klog.KObj(oldRevision), err)
		return nil, err
	}
	newRolesRevisions, err := c.splitComponentControllerRevision(newRevision)
	if err != nil {
		klog.Errorf("fail to split new role controllerRevision: %v, err: %v", klog.KObj(newRevision), err)
		return nil, err
	}
