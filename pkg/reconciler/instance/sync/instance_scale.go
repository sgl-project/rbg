...
func (c *realControl) Scale(ctx context.Context, updateInstance *v1alpha1.Instance, currentRevision, updateRevision *apps.ControllerRevision,
	...
	diffRes, err := c.calculateDiffsWithExpectation(updateInstance, currentRevision, updateRevision, revisions, pods)
	if err != nil {
		return true, err
	}
	if diffRes.toScaleNum > 0 {
		return c.createPods(updateInstance, diffRes.toScaleRoleIDS, updateRevision.Name)
	}
	if diffRes.toDeleteNum > 0 {
		return c.deletePods(updateInstance, diffRes.toDeletePod)
	}
	return false, nil
}
...
func (c *realControl) calculateDiffsWithExpectation(updateInstance *v1alpha1.Instance,
	...
	if updateInstance.Spec.PodGroupPolicy != nil {
		for i := range pods {
...
func (c *realControl) createPods(updateInstance *v1alpha1.Instance, expectedCreations map[string]sets.Set[int32], updateRevision string) (bool, error) {
	coreControl := instancecore.New(updateInstance)
	var newPods []*v1.Pod
	...
	_, err := instanceutil.DoItSlowly(toCreatePodNum, initialBatchSize, func() error {
		...
		if createErr := c.createOnePod(updateInstance, pod); createErr != nil {
			return createErr
		}
...
func (c *realControl) deletePods(instance *v1alpha1.Instance, podsToDelete []*v1.Pod) (bool, error) {
	var modified bool
...
func (c *realControl) createOnePod(instance *v1alpha1.Instance, pod *v1.Pod) error {
