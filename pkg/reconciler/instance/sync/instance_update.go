...
func (c *realControl) Update(ctx context.Context, instance *v1alpha1.Instance, _, updateRevision *apps.ControllerRevision, revisions []*apps.ControllerRevision, pods []*v1.Pod) (time.Duration, error) {
	...
	for i := range pods {
		if res := c.inplaceControl.Refresh(pods[i], coreControl.GetUpdateOptions()); res.RefreshErr != nil {
			klog.Errorf("Instance %s/%s failed to update pod %s condition for inplace: %v", instance.Namespace, instance.Name, pods[i].Name, res.RefreshErr)
...
func (c *realControl) updatePod(ctx context.Context, instance *v1alpha1.Instance,
	...
	logger.Info("Instance %s/%s can not update Pod %s in-place, so it will back off to ReCreate", instance.Namespace, instance.Name, pod.Name)
