...
type StatusUpdater interface {
	UpdateInstanceStatus(instance *v1alpha1.Instance, newStatus *v1alpha1.InstanceStatus, pods []*v1.Pod) error
}
...
func (r *realStatusUpdater) UpdateInstanceStatus(instance *v1alpha1.Instance, newStatus *v1alpha1.InstanceStatus, pods []*v1.Pod) error {
	newInnerCondition := make([]v1alpha1.InstanceCondition, 0, len(newStatus.Conditions))
	for _, c := range newStatus.Conditions {
		newInnerCondition = append(newInnerCondition, c)
	}
	r.calculateStatus(instance, newStatus, newInnerCondition, pods)
	var updateStatusErr error
	if r.inconsistentStatus(instance, newStatus) {
		updateStatusErr = r.updateStatus(instance, newStatus)
	}
	updatePodsConditionErr := r.updatePodsLifeCycle(instance, pods)
...
func (r *realStatusUpdater) calculateStatus(instance *v1alpha1.Instance, newStatus *v1alpha1.InstanceStatus,
	newInnerConditions []v1alpha1.InstanceCondition, pods []*v1.Pod) {
	...
	r.setInstanceConditions(instance, newStatus, newInnerConditions)
}

var innerInstanceConditions = sets.New[v1alpha1.InstanceConditionType](
	v1alpha1.InstanceReady,
	v1alpha1.InstanceFailedUpdate,
	v1alpha1.InstanceFailedScale,
	v1alpha1.InstanceAllPodsReady,
)
...
func (r *realStatusUpdater) setInstanceConditions(instance *v1alpha1.Instance, newStatus *v1alpha1.InstanceStatus,
	...
	for _, c := range instance.Status.Conditions {
		if innerInstanceConditions.Has(c.Type) {
...
func getInstanceReadyCondition(instance *v1alpha1.Instance, podsReady v1alpha1.InstanceCondition, inplace *v1alpha1.InstanceCondition) v1alpha1.InstanceCondition {
	...
	if containsInstanceInplaceUpdateReadinessGates(instance) {
		if inplace != nil {
			if inplace.Status == v1.ConditionFalse {
				instanceReady.Reason = "InplaceNotReady"
	...
	if podsReady.Status == v1.ConditionFalse {
...
func (r *realStatusUpdater) updateStatus(instance *v1alpha1.Instance, newStatus *v1alpha1.InstanceStatus) error {
	instance.Status = *newStatus
	return r.Status().Update(context.TODO(), instance)
}

func (r *realStatusUpdater) updatePodsLifeCycle(instance *v1alpha1.Instance, pods []*v1.Pod) error {
	podsChan := make(chan *v1.Pod, len(pods))
	for _, p := range pods {
		podsChan <- p
	}
	instanceMarkedNotReady := isInstanceMarkedNotReady(instance)
	markNotReady := instanceMarkedNotReady
	if getInstanceReadyPolicy(instance) == v1alpha1.InstanceReadyOnAllPodReady {
		allPodRuntimeReady := r.allPodsRuntimeReady(instance)
		markNotReady = instanceMarkedNotReady || !allPodRuntimeReady
	}
	_, err := instanceutils.DoItSlowly(len(pods), initialBatchSize, func() error {
		pod := <-podsChan
		if updated, gotPod, updateErr := r.lifecycleControl.UpdatePodLifecycle(instance, pod, markNotReady); updateErr != nil {
			return updateErr
		} else if updated {
...
func isInstanceMarkedNotReady(instance *v1alpha1.Instance) bool {
...
func (r *realStatusUpdater) allPodsRuntimeReady(instance *v1alpha1.Instance) bool {
	pods, err := r.getExpectOwnedPods(instance)
	if err != nil {
		return false
	}
	for _, pod := range pods {
		if !instanceutils.IsPodRuntimeReady(pod, 0) {
			klog.InfoS("pod runtime not ready", "pod", klog.KObj(pod))
			return false
		}
	}
	return true
