...
var getOwner = func(owner metav1.Object, schema *runtime.Scheme, c client.Client) (runtime.Object, error) {
	...
	return clientObj, c.Get(context.TODO(), client.ObjectKey{Namespace: owner.GetNamespace(), Name: owner.GetName()}, clientObj)
}
...
func (mgr *RefManager) release(obj metav1.Object) error {
	...
	if idx > -1 {
		runtimeObj, ok := obj.(client.Object)
		if !ok {
			return fmt.Errorf("can't remove Instance %v/%v (%v) owner reference: fail to cast to client.Object", obj.GetNamespace(), obj.GetName(), obj.GetUID())
		}

		obj.SetOwnerReferences(append(obj.GetOwnerReferences()[:idx], obj.GetOwnerReferences()[idx+1:]...))
		if err := mgr.updateOwnee(runtimeObj); err != nil {
			return fmt.Errorf("can't remove Instance %v/%v (%v) owner reference %v/%v (%v): %v",
