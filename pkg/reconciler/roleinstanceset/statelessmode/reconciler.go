...
func (r *ReconcileInstanceSet) truncateInstancesToDelete(set *workloadsv1alpha2.RoleInstanceSet, instances []*workloadsv1alpha2.RoleInstance) error {
	...
	for _, name := range set.Spec.ScaleStrategy.RoleInstanceToDelete {
		if existingInstances.Has(name) {
			newInstancesToDelete = append(newInstancesToDelete, name)
		}
	}

	if len(newInstancesToDelete) == len(set.Spec.ScaleStrategy.RoleInstanceToDelete) {
		return nil
	}

	newSet := set.DeepCopy()
	newSet.Spec.ScaleStrategy.RoleInstanceToDelete = newInstancesToDelete
