...
func newReconciler(mgr manager.Manager) reconcile.Reconciler {
	...
	instanceInformer, err := cacher.GetInformerForKind(context.TODO(), workloadsv1alpha2.GroupVersion.WithKind("RoleInstance"))
	if err != nil {
		klog.ErrorS(err, "Failed to get informer for Instance")
		return nil
	}

	roleInstanceSetLister := instancelisters.NewRoleInstanceSetLister(instanceSetInformer.(toolscache.SharedIndexInformer).GetIndexer())
	roleInstanceLister := instancelisters.NewRoleInstanceLister(instanceInformer.(toolscache.SharedIndexInformer).GetIndexer())
	...
	return &ReconcileStatefulInstanceSet{
		control: NewDefaultStatefulInstanceSetControl(
			...
			NewRealStatefulInstanceSetStatusUpdater(sigsruntimeClient, roleInstanceSetLister),
			utilhistory.NewHistory(sigsruntimeClient),
			recorder,
		),
		instanceControl:       &realInstanceControl{sigsruntimeClient, recorder},
		roleInstanceLister:    roleInstanceLister,
		roleInstanceSetLister: roleInstanceSetLister,
	}
}
...
type ReconcileStatefulInstanceSet struct {
	...
	// roleInstanceLister is able to list/get role instances from a shared informer's store
	roleInstanceLister instancelisters.RoleInstanceLister
	// roleInstanceSetLister is able to list/get role instance sets from a shared informer's store
	roleInstanceSetLister instancelisters.RoleInstanceSetLister
}

// +kubebuilder:rbac:groups=workloads.x-k8s.io,resources=instances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=workloads.x-k8s.io,resources=instances/status,verbs=get;update;patch;create;delete;list;watch
...
// +kubebuilder:rbac:groups=workloads.x-k8s.io,resources=instancesets/status,verbs=get;update;patch;create;delete;list;watch
...
func (ssc *ReconcileStatefulInstanceSet) adoptOrphanRevisions(ctx context.Context, set *workloadsv1alpha2.RoleInstanceSet) error {
	...
	if len(orphanRevisions) > 0 {
		fresh := &workloadsv1alpha2.RoleInstanceSet{}
...
func (ssc *ReconcileStatefulInstanceSet) getInstancesForInstanceSet(ctx context.Context, set *workloadsv1alpha2.RoleInstanceSet, selector labels.Selector) ([]*workloadsv1alpha2.RoleInstance, error) {
	...
	instances, err := ssc.roleInstanceLister.RoleInstances(set.Namespace).List(labels.Everything())
	if err != nil {
		return nil, err
	}

	filter := func(instance *workloadsv1alpha2.RoleInstance) bool {
...
func (ssc *ReconcileStatefulInstanceSet) syncStatefulInstanceSet(ctx context.Context, set *workloadsv1alpha2.RoleInstanceSet, instances []*workloadsv1alpha2.RoleInstance) error {
