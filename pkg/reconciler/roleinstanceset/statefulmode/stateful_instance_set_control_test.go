...
func TestEarlyRollbackKeepsSurgeUntilReplacementIsReady(t *testing.T) {
	...
	reconcile := func(instances ...*workloadsv1alpha2.RoleInstance) *workloadsv1alpha2.RoleInstanceSetStatus {
		...
		// The real status updater persists this status before the next
		// reconcile; emulate that so the restart case below is realistic.
		set.Status = *status
		return status
	}
	...
	if !hasEarlyRollbackReplacementCondition(set) {
		t.Fatalf("early rollback did not persist its replacement signal")
	}

	// Simulate a controller restart or leader change after the stale base is
	// gone but before its replacement is Ready. The durable status condition
	// must restore the in-memory signal and keep the surge in range.
	resetEarlyRollbackReplacementUIDs()
	...
	if !hasEarlyRollbackReplacementCondition(set) {
		t.Fatalf("early rollback replacement signal was lost across restart")
	}
	if _, ok := earlyRollbackReplacementUIDs.Load(key); !ok {
		t.Fatalf("early rollback condition did not hydrate the in-memory signal")
	}
	...
	if hasEarlyRollbackReplacementCondition(set) {
		t.Fatalf("completed early rollback retained its replacement condition")
	}
}
