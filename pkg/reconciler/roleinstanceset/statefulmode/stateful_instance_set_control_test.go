...
func TestComputeTopologyRollback(t *testing.T) {
	tests := []struct {
		name        string
		instances   []*workloadsv1alpha2.RoleInstance
		partition   int32
		...
		{
			name:        "healthy stale base needs surge",
			instances:   []*workloadsv1alpha2.RoleInstance{buildInst("s", 1, testOldRev, true, true)},
			wantRollout: true, wantSurge: 1,
		},
		{
			name:        "unhealthy stale base still needs an update",
			instances:   []*workloadsv1alpha2.RoleInstance{buildInst("s", 1, testOldRev, false, true)},
			wantRollout: true,
		},
		{
			name:      "stale base below partition stays unchanged",
			instances: []*workloadsv1alpha2.RoleInstance{buildInst("s", 1, testOldRev, true, true)},
			partition: 2,
		},
		{
			name:        "stale base at partition needs an update",
			instances:   []*workloadsv1alpha2.RoleInstance{buildInst("s", 1, testOldRev, true, true)},
			partition:   1,
			wantRollout: true, wantSurge: 1,
		},
		{
			name: "stale surge alone does not start a rollout",
			instances: []*workloadsv1alpha2.RoleInstance{
				buildInst("s", 1, testUpdateRev, true, true), buildInst("s", 2, testOldRev, true, true),
			},
		},
		{
			name:      "base with a delete timestamp needs no new update",
			instances: []*workloadsv1alpha2.RoleInstance{withTerminating(buildInst("s", 1, testOldRev, true, true))},
		},
		{
			name:      "uncreated stale base needs no update",
			instances: []*workloadsv1alpha2.RoleInstance{nil, buildInst("s", 1, testOldRev, true, false)},
		},
		{
			name:      "unhealthy base at target does not start a rollout",
			instances: []*workloadsv1alpha2.RoleInstance{buildInst("s", 1, testUpdateRev, false, true)},
		},
		{
			name: "keep surge while base is absent",
			instances: []*workloadsv1alpha2.RoleInstance{
		...
		{
			name: "keep surge until the new base is ready",
			instances: []*workloadsv1alpha2.RoleInstance{
				buildInst("s", 1, testUpdateRev, false, true), buildInst("s", 2, testUpdateRev, true, true),
			},
			wantRollout: true, wantSurge: 1,
		},
		{
			name: "release surge after base is ready",
			instances: []*workloadsv1alpha2.RoleInstance{
				buildInst("s", 1, testUpdateRev, true, true), buildInst("s", 2, testUpdateRev, true, true),
			},
		},
		{
			name:      "pause does not allocate surge",
			instances: []*workloadsv1alpha2.RoleInstance{buildInst("s", 1, testOldRev, true, true)},
			paused:    true,
		},
		{
			name: "pause preserves existing surge during rollback",
			instances: []*workloadsv1alpha2.RoleInstance{
				buildInst("s", 1, testOldRev, true, true), buildInst("s", 2, testUpdateRev, true, true),
			},
			paused: true, wantSurge: 1,
		},
		{
			name: "pause preserves surge until base is ready",
			instances: []*workloadsv1alpha2.RoleInstance{
				buildInst("s", 1, testUpdateRev, false, true), buildInst("s", 2, testUpdateRev, true, true),
			},
			paused: true, wantSurge: 1,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			set.Spec.UpdateStrategy.Partition = ptr.To(intstrutil.FromInt32(tt.partition))
			set.Spec.UpdateStrategy.Paused = tt.paused
			instances := append([]*workloadsv1alpha2.RoleInstance{buildInst("s", 0, testUpdateRev, true, true)}, tt.instances...)
			topo, err := computeTopology(set, instances, testUpdateRev, testUpdateRev)
			if err != nil {
				t.Fatal(err)
			}
			if topo.inRollout != tt.wantRollout || topo.activeSurge != tt.wantSurge || topo.endOrdinal != 2+tt.wantSurge {
				t.Fatalf("rollout=%v surge=%d end=%d, want rollout=%v surge=%d end=%d",
					topo.inRollout, topo.activeSurge, topo.endOrdinal, tt.wantRollout, tt.wantSurge, 2+tt.wantSurge)
			}
		})
	}
}
...
func TestUpdateStatefulInstanceSetRetriesUnhealthyRollout(t *testing.T) {
	...
	set.Spec.RoleInstanceTemplate.Components[0].Template.Spec.Containers[0].ReadinessProbe = &v1.Probe{
	...
	set.Spec.RoleInstanceTemplate.Components[0].Template.Spec.Containers[0].ReadinessProbe = nil
	...
	instances := []*workloadsv1alpha2.RoleInstance{
		...
		buildInst(set.Name, 1, currentRev.Name, false, true),
	}
	...
	assertWait := func(wait, upperBound time.Duration) {
		t.Helper()
		if wait <= 0 || wait > upperBound {
			t.Fatalf("retry = %v, want a positive delay of at most %v", wait, upperBound)
		}
		if len(objectManager.deleted) != 0 {
			t.Fatalf("instances deleted before the health window expired: %v", objectManager.deleted)
		}
	}

	// The first pass must protect both instances and request another check.
	...
	if remaining := time.Until(firstObserved.Add(stableUnhealthyDuration)); wait < remaining {
	...
	deleted := sets.New(objectManager.deleted...)
	for _, inst := range instances {
		if !deleted.Has(inst.Name) {
			t.Fatalf("rollout did not resume after the health window: deleted %v", objectManager.deleted)
		}
	}
}
