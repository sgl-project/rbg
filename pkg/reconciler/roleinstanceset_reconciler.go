...
func (r *RoleInstanceSetReconciler) constructRoleInstanceSetApplyConfiguration(
	...
	roleInstanceTemplateConfig := workloadsv1alpha2client.RoleInstanceTemplate().
		WithRestartPolicy(
			workloadsv1alpha2client.RestartPolicyConfig().
...
func allocateRoleScopedPortAnnotations(
	...
	for _, component := range components {
		...
		if err != nil {
			return fmt.Errorf("failed to parse port allocator config for component %s: %w", *component.Name, err)
		}
		...
		if needCreate {
			...
			if err != nil {
				return fmt.Errorf("failed to allocate role-scoped ports for component %s: %w", *component.Name, err)
			}
