...
| `Complete`       | True   | `NoNodesMatched`               | No nodes matched the target configuration; job completed immediately. `targetNodes` selectors that match nothing, or a `targetRoleBasedGroup` whose selected roles have no Pods at all, complete this way. A `targetRoleBasedGroup` with unscheduled Pods in its selected roles does **not**: the Warmup waits and sets `TargetReady=False` instead. |
...
| `TargetReady`    | False  | `TargetPodsNotScheduled`       | The referenced RoleBasedGroup exists but some Pods in the Warmup’s selected roles have no node assigned yet. The Warmup waits instead of completing.                                             |
| `TargetReady`    | True   | `RoleBasedGroupReady`          | The target RoleBasedGroup is observable and all Pods in the Warmup’s selected roles are scheduled; warmup proceeds.                                                                                    |
