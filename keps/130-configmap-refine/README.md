...
- Redundant content: each role of one RBG has a configmap while the content is the same
...
roles:
  <role-name>:
    ...
    size: <number-of-size-of-an-instance>
...
### Configmap Injection
Currently, the default injector creates and injects a configmap per role when each pod template of role manifested,
which leads to redundant configmaps for one single RBG.
```go
cmApplyConfig := coreapplyv1.ConfigMap(rbg.GetWorkloadName(role), rbg.Namespace).
```
A better approach is to create one RBG-level ConfigMap, named after the RBG, during one RBG reconcile loop, once workload
names are deterministically known. The injector would then only mount this shared ConfigMap for each role,
instead of creating and injecting a separate ConfigMap per role.

Therefore, moving ConfigMap creation to `reconcileRoles` in `rolebasedgroup_controller.go` is reasonable.
The shared ConfigMap can be reconciled in each RBG reconcile loop to keep its content up to date. Controller owns create/update, injector mount-only
...
roles:
  <role-name>:
    ...
    excludes: [<list-of-excluded-indices>] # Optional
```
For instance: one RBG with 4 instances, 1 router, 2 encoder, 3 prefill, 2 decode (excluding role index 1), the config file is like below:
```yaml
...
roles:  
   ...
   prefill:
     ...
     size: 2 # each replica has 2 pods
   decode: 
     replicas: 2 # two replicas, but role index 1 is excluded, so only decode-0 is included
     excludes: [1] # Optional, exist when rolling update or scaling down
     size: 4 # each replica has 4 pods
```
...
Because `id` is randomly generated by `getOrGenInstanceID()`, Deployment pod FQDNs cannot be deterministically derived at reconciliation time.
Therefore, this ConfigMap format does not support per-pod service discovery for Deployment workloads, and the controller will
not create nor mount any configmap for these RBGs.


### Runtime adaptation
The patio runtime python package, handling registration and lora adaption, may also need to add a new logic for parsing the new config.
...
The new format may break compatibility with existing systems that rely on the current configuration format. Therefore,
the controller should not create or mount any new configmap for exsisting RBG, nor edit exsisting configmaps contents.
