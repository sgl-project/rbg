...
The shared ConfigMap can be reconciled in each RBG reconcile loop to keep its content up to date.
...
roles:  
   ...
   decode: 
     replicas: 2
     excludes: [1] # Optional, exist when rolling update
     size: 2
```
...
Therefore, this ConfigMap format does not support per-pod service discovery for Deployment workloads.
