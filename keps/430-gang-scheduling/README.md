...
This KEP introduces two-level gang scheduling for RBG:

1. **RBG level** (Group level): All pods created by the RBG must be scheduled simultaneously, otherwise all wait.
2. **Role level** (SubGroup level): Each role in the RBG can independently configure a minimum number of replicas to schedule, e.g., prefill must schedule at least 2 replicas, decode at least 1 replica.

This feature is configured via CoordinatedPolicy's `scheduling.gang` strategy and is **not bound to a specific scheduler**. The RBG controller passes the strategy directly to the scheduler implementation (`BasicGangScheduling`), which translates it into the scheduler's own API. The first complete implementation will support subGroupPolicy, with extensibility for other schedulers in the future.

## Motivation

Currently, RBG's gang scheduling only supports the RBG level (minMember), meaning all pods must be satisfied simultaneously or all wait.
...
1. Implement two-level gang scheduling: RBG level and Role level
...
3. Implementing new schedulers (only existing Volcano and scheduler-plugins are supported)
...
Effect: The scheduler creates a PodGroup for the RBG, where the prefill subgroup must have at least 2 complete replicas and the decode subgroup at least 1 complete replica. Both must be satisfied simultaneously for scheduling to proceed.

> **Note**: SubGroup-level gang requires scheduler support for this capability. Schedulers that do not support subGroup will reject the gang strategy and fall back to full gang (see [Webhook and Capability Validation](#webhook-and-capability-validation)).

#### Story 2: Basic Gang Scheduling

The user only needs full gang scheduling (all pods scheduled simultaneously or all wait), without subGroup-level constraints:
...
graph TD
    ...
    C --> E{"BasicGangScheduling<br/>(pluggable)"}
    E -->|"subGroup supported"| F["PodGroup + subGroupPolicy"]
    E -->|"subGroup not supported"| G["PodGroup + minMember<br/>(reject + safety net)"]
...
type GangSchedulingStrategy struct {
    ...
    // that must be scheduled together (gang scheduling).
    //
    // When non-empty, only the roles listed in this map participate in
    // per-role gang scheduling. Roles absent from this map are excluded
    // from gang constraints and scheduled normally.
    //
...
Replaced with two interfaces, using Go interface type assertion for capability discovery (an alternative to adding a `SupportsSubGroups() bool` method):

```go
// BasicGangScheduling is the base interface that all scheduler implementations
// must satisfy. Handles basic gang scheduling (minMember = GetGroupSize)
// and pod scheduler field injection.
type BasicGangScheduling interface {
    // GetSchedulerName returns the scheduler name to set on pod.spec.schedulerName.
    GetSchedulerName() string

    // ReconcilePodGroup creates/updates the PodGroup with minMember = GetGroupSize().
    // Called for basic gang (no minReplicas or annotation compat).
    ReconcilePodGroup(
        ctx context.Context,
    ...
    // InjectPodSchedulingFields injects scheduler-specific fields for basic gang:
    // - pod.spec.schedulerName (all pods)
    // - PodGroup annotation/label (all pods, since basic gang = all roles participate)
...
// RoleBasedGangScheduling is an optional interface for schedulers that
// support per-role gang constraints (e.g. Volcano subGroupPolicy).
// Implementations must provide ReconcilePodGroupWithSubGroup, which the
// controller calls via type assertion when gangStrategy.MinReplicas is non-empty.
//
//   if rbs, ok := mgr.(RoleBasedGangScheduling); ok {
//       rbs.ReconcilePodGroupWithSubGroup(ctx, rbg, gangStrategy, ...)
//   }
type RoleBasedGangScheduling interface {
    BasicGangScheduling

    // ReconcilePodGroupWithSubGroup creates/updates the PodGroup with per-role
    // subGroup constraints from gangStrategy.MinReplicas.
    ReconcilePodGroupWithSubGroup(
        ctx context.Context,
        rbg *workloadsv1alpha2.RoleBasedGroup,
...
- `pkg/scheduler/volcano/` — implements `RoleBasedGangScheduling` (supports subGroupPolicy)
- `pkg/scheduler/k8s-scheduler-plugin/` — implements only `BasicGangScheduling` (minMember-only)

The controller selects the call path based on whether `gangStrategy.MinReplicas` is non-empty: if non-empty, calls `ReconcilePodGroupWithSubGroup` (requires `RoleBasedGangScheduling`); otherwise calls `ReconcilePodGroup` (basic gang, `minMember = GetGroupSize()`).

#### Interface Migration Impact

The rename from `InjectPodGroupLabels(rbg, pts)` to `InjectPodSchedulingFields(rbg, role, pts)` adds a `role *RoleSpec` parameter. Existing callers must be updated:

- `pkg/reconciler/pod_reconciler.go` — currently calls `r.podGroupManager.InjectPodGroupLabels(rbg, podTemplateApplyConfiguration)` in `buildPodTemplateSpec`. The `role` variable is already in scope, so the migration is straightforward: pass `role` as the second argument.
- `pkg/reconciler/roleinstanceset_reconciler.go` — holds a `podGroupManager` field via `SetPodGroupManager`. Verify whether its internal pod-template construction path also calls `InjectPodGroupLabels` and update accordingly.
...
Gang scheduling is a safety guarantee, not a convenience feature. When the scheduler does not support subGroup, **silent degradation is dangerous**: degrading to the gang minimum (Σ minReplicas × subGroupSize) lowers the gang threshold, allowing partial scheduling of wrong roles (e.g., all prefill, zero decode), which is less safe than not configuring gang scheduling at all.

Therefore, a **Webhook rejection** strategy is adopted: when `scheduling.gang.minReplicas` is non-empty in the CoordinatedPolicy and the current scheduler does not support subGroup, the Webhook directly rejects creation/update.
...
Implementation: pass the scheduler instance (`BasicGangScheduling`) into CoordinatedPolicy's `CustomValidator`, and use type assertion to check whether it implements `RoleBasedGangScheduling`, consistent with the existing `enableDeprecatedWorkloadTypes` passing pattern.
...
| Scheduler capability | When `minReplicas` is non-empty and the scheduler does not support subGroup | Reject creation/update |
...
Degrading to the gang minimum (6) lowers minMember from the full count (17), making partial scheduling easier — the scheduler might select 6 prefill pods to satisfy the gang, but zero decode pods are scheduled, which is exactly the scenario the user wants to avoid.

| Scheduler | Implements `RoleBasedGangScheduling` | Notes |
|---|---|---|
| Volcano | ✓ | Go code implements the interface; cluster must have Volcano ≥ 1.14 installed (PodGroup CRD with `subGroupPolicy` field), otherwise PodGroup creation is rejected by the API server |
| scheduler-plugins | ✗ | PodGroup only has `minMember` and `ScheduleTimeoutSeconds` |
| Future schedulers | Depends on implementation | Self-declared at implementation time |

#### Runtime Safety Net

Under normal circumstances, the Webhook has already intercepted at creation time. However, if the user switches `--scheduler-name` after creating the CoordinatedPolicy (e.g., from volcano to scheduler-plugins), the controller detects during reconcile that `gangStrategy.MinReplicas` is non-empty but the current scheduler does not implement `RoleBasedGangScheduling`:
...
RBG Reconcile
  ...
  │
  ├── 4. Call scheduler interface based on strategy
  │     if gangStrategy != nil && len(gangStrategy.MinReplicas) > 0:
  │         if rbs, ok := mgr.(RoleBasedGangScheduling); ok:
  │             rbs.ReconcilePodGroupWithSubGroup(ctx, rbg, gangStrategy, ...)
  │         else:
  │             // Runtime safety net: Webhook should have intercepted, return error here
  │             // No degradation (degradation violates user intent), no PodGroup created
  │             return error + RBG status condition
  │     else:
  │         mgr.ReconcilePodGroup(ctx, rbg, ...)
  │
...
| Field | Description |
|---|---|
| `spec.schedulerName` | When gang scheduling is enabled, the controller automatically sets this via `BasicGangScheduling.GetSchedulerName()` (e.g., `"volcano"`). Users do not need to manually set it in the pod template. All pods get schedulerName; only gang-participating roles' pods additionally get PodGroup annotation |
...
**Pod annotations** (existing, scheduler-specific):
...
| CoordinatedPolicy with scheduling.gang (with minReplicas) + scheduler supports subGroup | Pass GangSchedulingStrategy, enable subGroup-level gang |
| CoordinatedPolicy with scheduling.gang (with minReplicas) + scheduler does not support subGroup | Webhook rejects creation/update |
...
3. Logic for extracting gang strategy from CoordinatedPolicy, including:
   - Single policy rule
   - Multiple policy rules with overlapping roles (take minimum)
   - Roles not in minReplicas do not participate in gang
4. Volcano manager correctness in translating `GangSchedulingStrategy` to PodGroup subGroupPolicy.
5. Degrade to minMember-only when `minReplicas` is empty.
6. Type assertion capability discovery: Volcano satisfies `RoleBasedGangScheduling`, scheduler-plugins does not.
7. Webhook scheduler capability validation: CoordinatedPolicy with minReplicas rejected when scheduler does not implement `RoleBasedGangScheduling`.
...
3. Delete CoordinatedPolicy, verify PodGroup degrades to minMember-only.
...
- [ ] Volcano manager subGroupPolicy support implementation
- [ ] Interface split (PodGroupManager → BasicGangScheduling + RoleBasedGangScheduling)
- [ ] `GetSchedulerName()` method + type assertion capability discovery implementation
...
1. **Scheduler capability disparity**: SubGroup-level gang depends on scheduler support for subGroupPolicy (Volcano ≥ 1.14). When not supported, the Webhook rejects creation of CoordinatedPolicy with minReplicas. Users receive clear error feedback at creation time.
