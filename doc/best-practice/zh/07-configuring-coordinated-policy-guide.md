...
- `prefill` 和 `decode` 按接近的进度创建 Pod，每次创建 1 decode 和 2 prefill，Prefill 总是稍晚就绪，且 Prefill 就绪后才继续下一批创建
...
### 步骤 1：创建 CoordinatedPolicy 和多角色 RBG（协作升级）
...
- `rollingUpdate.maxSkew: "25%"`：两个角色之间的更新进度差异最多为 25%（本案例中，最大差值即 1 个实例）
...
- 更新过程中，两个角色之间的更新进度差异不超过 25%（本案例中，最大差值即 1 个实例）
