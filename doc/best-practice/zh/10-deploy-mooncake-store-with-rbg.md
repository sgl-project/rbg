...
```plain
Master Metrics: Storage: 1.23 GB / 30.00 GB (4.1%) | Keys: 8547 (soft-pinned: 0)
```

`Storage` 和 `Keys` 的非零值说明 KV Cache 已成功 offload 到 Mooncake Store。

### 步骤 3：基准测试验证性能提升
使用 SGLang 内置的 `bench_serving` 工具进行多轮对话基准测试，对比启用和未启用 Mooncake 的 SLO 指标：
...
#### 预期结果
基于 KEP-74 的基准测试数据（Qwen3-32B, 3 Store × 10 GiB L3 缓存）：

| 指标 | 无 Mooncake | 有 Mooncake | 提升 |
| --- | --- | --- | --- |
| **首轮对话 TTFT** | 4808 ms | 5017 ms | 基本持平（首轮无缓存可复用） |
| **多轮对话 TTFT** | 1172 ms | 94 ms | **-91.94%** |
| **多轮对话 ITL** | 140 ms | 58 ms | **-58.83%** |
| **多轮对话吞吐量** | 1385 tok/s | 1936 tok/s | **+39.80%** |


> **说明**：首轮对话无可复用的 KV Cache，性能基本持平。多轮对话中，历史上下文的 KV Cache 从 Mooncake L3 缓存中取回，避免了重新计算，TTFT 大幅降低。
>

### 步骤 4：验证 KV Cache 跨实例复用（可选）
