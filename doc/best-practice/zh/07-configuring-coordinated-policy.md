...
| `spec.policies[].strategy.scaling` | object | 否 | 协作伸缩策略（对首次部署与后续扩容均生效） |
| `spec.policies[].strategy.rollingUpdate` | object | 否 | 协作升级策略 |


> **说明**：`scaling` 和 `rollingUpdate` 可以同时配置，也可以单独使用。`scaling` 对首次部署和后续扩容均生效，会持续限制角色间的扩容进度差异；不影响缩容行为。一个 CoordinatedPolicy 可以包含多条规则，分别作用于不同的角色组。
>
...
> **说明**：协作伸缩对首次部署和后续扩容均生效：扩容时同样会按 `maxSkew` 限制角色间的进度差异，避免某一角色领先过多；策略持续生效，不会在首次部署完成后失效。缩容不受该策略限制。
>
...
| 参数 | 类型 | 是否必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| `strategy.scaling.maxSkew` | intOrString | 否 | `"100%"` | 允许的角色间部署进度偏差。仅支持百分比格式（如 `"10%"`）；不支持整数（如 `2`），设置整数会导致解析失败，所有角色的副本都不会被创建 |
| `strategy.scaling.progression` | string | 否 | 无 | 创建节奏：`OrderScheduled` 或 `OrderReady`。省略时批次门控不生效，副本创建不会等待 Pod 调度或就绪，建议显式设置 |


### maxSkew 的工作原理
`maxSkew` 控制各角色的创建进度差异。Controller 计算每个角色的部署进度（已创建的副本数 / 目标副本数），确保任意两个角色的进度差异不超过 `maxSkew`。

假设 Prefill 目标副本数为 5，Decode 目标副本数为 10，`maxSkew: "10%"`：

#### 协作伸缩过程（maxSkew: 10%）

| 批次 | Prefill 进度 | Decode 进度 | 进度差异 | 说明 |
| --- | --- | --- | --- | --- |
| 1 | `0 → 1`（20%） | `0 → 1`（10%） | 10% | 两个角色按比例同时开始创建，差异达到 maxSkew 上限 |
| 2 | 保持 1（20%） | `1 → 2`（20%） | 0% | 进度上限 = 最慢未完成角色进度 + maxSkew = 0.2，Prefill 已达上限被暂缓，等待 Decode 追上 |
| 3 | `1 → 2`（40%） | `2 → 3`（30%） | 10% | 差异重新回到上限，Prefill 恢复创建 |

以此类推，两个角色始终保持按比例渐进式创建。如果 Prefill 创建过快，进度差异超过 10%，Controller 会暂缓 Prefill 的创建，等待 Decode 跟上。
...
| `strategy.rollingUpdate.maxUnavailable` | intOrString | 否 | - | 协作更新过程中，单个角色允许不可用的最大实例数，百分比按各角色自身副本数换算。该值会覆盖角色自身的 `rolloutStrategy.rollingUpdate.maxUnavailable` |
| `strategy.rollingUpdate.partition` | intOrString | 否 | - | 协作更新的分区值，百分比按各角色自身副本数换算（如 `"50%"` 在 7 副本角色上为 4，在 3 副本角色上为 2）。该值仅作为下限：若 `maxSkew` 推导出更大的分区值，以后者为准 |
...
3. **协调更新顺序**：如果 Prefill 更新过快，Controller 暂缓 Prefill 的更新，等待 Decode 跟上
...
进度差异：`57% - 33% = 24%`，超过 `10%`，Controller 暂缓 Prefill 的更新，优先推进 Decode 的更新。
...
+ 为 RBG 服务配置弹性伸缩策略
+ [原地升级与原地调度](./04-configuring-inplace-update-and-scheduling-policies.md)
