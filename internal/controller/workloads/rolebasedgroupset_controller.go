...
func (r *RoleBasedGroupSetReconciler) reconcileRolling(
	...
	// surgeDone drives both surge reclamation (step 2) and surge warm-up (step 4), so the
	// two steps can never fight over a surge group within one reconcile or across
	// reconciles. The rollout is done for surge purposes once every base group is serving
	// and either the whole set carries the template, or the partition-scoped rollout has
	// finished with a non-empty scope. partition == replicas is the one configuration that
	// keeps surge: its rollout scope is empty, so only real convergence releases the
	// standing canary.
	converged := r.rolloutComplete(rbgset, children, replicas, 0)
	complete := converged
	if params.partition > 0 {
		complete = r.rolloutComplete(rbgset, children, replicas, params.partition)
	}
	surgeDone := converged || (complete && params.partition < replicas)
	...
	keptSurge, err := r.reclaimSurgeGroups(ctx, replicas, children, params, surgeDone)
	if err != nil {
		return err
	}
	...
	// 4. Warm up surge capacity, but only when it can actually widen the delete
	// parallelism: the budget is maxUnavailable + readySurge, so once maxUnavailable alone
	// covers the in-scope work, a surge group would be created and later reclaimed without
	// ever gating anything. The standing canary (partition == replicas) is the exception:
	// there the surge group itself is the point, not the budget.
	inScope := 0
	...
	surgeUseful := inScope > params.maxUnavailable || (params.partition >= replicas && len(outdated) > 0)
	if !params.paused && !surgeDone && surgeUseful && len(keptSurge) < params.maxSurge {
...
// reclaimSurgeGroups deletes the surge groups that exceed the surge budget, or all of them
// once the rollout is done for surge purposes, and returns the ones still serving rollout
// capacity.
func (r *RoleBasedGroupSetReconciler) reclaimSurgeGroups(
	ctx context.Context, replicas int, children classifiedChildren, params rollingParams, surgeDone bool,
	...
	for ordinal, rbg := range children.surge {
		if ordinal >= replicas+params.maxSurge || surgeDone {
...
// rolloutComplete reports whether every base ordinal exists and is serving, and every
// ordinal at or above the partition carries the current template. Callers choose the
// partition: the configured value answers "is the current rollout batch done", and 0
// answers "has the whole set converged on the template". Serving is required of every base
// group in both readings, so a group that was just recreated and is still coming up keeps
// the answer false — withdrawing surge capacity at that moment would leave the set short
// of ready groups.
...
func (r *RoleBasedGroupSetReconciler) onlyReplicasChanged(
	...
	// Deep copies plus legacy-strategy normalization, for the same reason rolesEqual does
	// both: a stored template can keep the legacy "Recreate" spelling while children carry
	// the webhook-normalized "RecreatePod", and that spelling-only delta must not read as
	// a real change here either — otherwise a replicas-only diff would be misrouted to
	// recreate on any cluster still holding the legacy spelling.
	childCopy := deepCopyRoles(child)
	templateCopy := deepCopyRoles(template)
	normalizeRolloutUpdateTypes(childCopy)
	normalizeRolloutUpdateTypes(templateCopy)

	byName := make(map[string]*workloadsv1alpha2.RoleSpec, len(childCopy))
	for i := range childCopy {
		byName[childCopy[i].Name] = &childCopy[i]
	}
	for i := range templateCopy {
		c, ok := byName[templateCopy[i].Name]
		if !ok {
			return false
		}
		// Compare with Replicas masked out; sharing the pointer makes the field equal.
		cc := *c
		cc.Replicas = templateCopy[i].Replicas
		if !reflect.DeepEqual(cc, templateCopy[i]) {
			return false
		}
	}
	return true
...
// updateStatus updates the status of the RoleBasedGroupSet. The counters follow the
// capacity/progress split used by Deployment and CloneSet: the capacity counters
// (replicas, readyReplicas) count every child including surge groups,
// while the progress counters (currentReplicas, updatedReplicas, updatedReadyReplicas)
// count only base ordinals.
func (r *RoleBasedGroupSetReconciler) updateStatus(
	...
	var total, readyTotal int32
	var baseTotal, current, updated, updatedReady int32
	...
	if readyTotal >= int32(replicas) {
		...
		meta.SetStatusCondition(&newStatus.Conditions, metav1.Condition{
			...
			Message: fmt.Sprintf(
				"Waiting for replicas to be ready (%d/%d)", readyTotal, replicas,
			),
		})
	}
...
// Template.Annotations. Only the enumerated controller-owned keys never count as drift: the
// RoleBasedGroup controller records discovery-config-mode on its own object, and treating
// that as drift makes the two controllers revert each other forever.
...
// systemManagedLabelKeys are the label keys the controllers own on a child RoleBasedGroup.
// Template syncing carries them over from the child instead of rebuilding them from the
// template, which is what keeps two controllers from fighting over the same object. Every
// other key, including other keys under the project prefix such as group-exclusive-topology,
// is user-facing and must propagate from the template like any other label.
var systemManagedLabelKeys = map[string]bool{
	constants.GroupSetNameLabelKey:  true,
	constants.GroupSetIndexLabelKey: true,
}

// systemManagedAnnotationKeys are the annotation keys the controllers own on a child
// RoleBasedGroup: the RoleBasedGroup controller records discovery-config-mode on its own
// object, and everything else under the project prefix is user-facing.
...
//
// The enumerated controller-owned keys are carried over from the child rather than rebuilt
// from the template, because they belong to the controllers. Replacing the whole map deletes
// what the RoleBasedGroup controller recorded there, which makes it write it again, and
// makes this controller see drift again.
